...
import com.lsy.bean.MessageBean;
import com.lsy.namespace.R;
import com.lsy.ui.MessageBoxListAdapter;
...
public class MessageBoxList extends Activity {
	...
	public void onCreate(Bundle savedInstanceState){
		...
		address=HomeContactActivity.member.getPhoneNum();
		TextView tv=(TextView) findViewById(R.id.topbar_title);
		tv.setText(HomeContactActivity.member.getDisplayName());
		...
		fasong.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
		        ...
		        sms.sendTextMessage(address, null, nei, null, null);
				init();
				neirong.setText("");
			}
		});
	...
	private void init(){
		...
		asyncQuery.startQuery(0, null, uri, projection, "address = " + address, null,
				"date asc");
	}
		...
		protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
			if (cursor != null && cursor.getCount() > 0) {
				...
				for (int i = 0; i < cursor.getCount(); i++) {
					...
					if(cursor.getInt(cursor.getColumnIndex("type")) == 1){ //判断是收件人还是发件人
						MessageBean d = new MessageBean(cursor.getString(cursor.getColumnIndex("address")), 
								date,
								cursor.getString(cursor.getColumnIndex("body")),
								R.layout.list_say_he_item);//显示短信内容
						list.add(d);
					}else{ 
						MessageBean d = new MessageBean(cursor.getString(cursor.getColumnIndex("address")),
								date,
