...
public class QuickAlphabeticBar extends ImageButton {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (selectIndex > -1 && selectIndex < letters.length) {// 防止越界
			String key = letters[selectIndex];//获取当前首字母
			if (alphaIndexer.containsKey(key)) {//如果当前联系人含有该字母，算出该字母在listview中第一次出现的位置
				int pos = alphaIndexer.get(key);
				if (mList.getHeaderViewsCount() > 0) {// 防止ListView有标题栏，本例中没有。
					this.mList.setSelectionFromTop(//设置ListView选中的位置，同时在Y轴设置一个偏移量
